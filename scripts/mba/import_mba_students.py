...
Then:
>>> from scripts.mba.import_mba_students import run_import
>>> run_import('old_data/mba/MBA_DATA.xlsx')

OR run directly:
poetry run python scripts/mba/import_mba_students.py --file "old_data/MBA_DATA.xlsx"
...
- MBA Exam (used to fetch session/semester)
- Result (Status e.g., PASS)
- Total (Marks Obtained)
- Subject Columns (Subject names as defined in MBASubject)
"""
...
NON_SUBJECT_COLUMNS = [
    ...
    'College', 'Batch', 'Session', 'Father Name', 'MBA Exam', 'Exam Center',
...
def run_import(file_path):
    ...
    for index, row in df.iterrows():
        ...
        if session_name not in cache['sessions']:
            cache['sessions'][session_name] = MBASession.objects.filter(name=session_name).first()
        ...
        if batch_name not in cache['batches']:
            ...
            if sess:
                cache['batches'][batch_name] = MBABatch.objects.filter(name=batch_name, session=sess).first()
            else:
        ...
        if not pd.isna(exam_name):
            for sub_col in subject_cols:
                ...
                if subject_name not in cache['subjects']:
                    cache['subjects'][subject_name] = MBASubject.objects.filter(name__iexact=subject_name).first()
                if not cache['subjects'][subject_name]:
                    # Also try to find by paper_code
                    subj = MBASubject.objects.filter(paper_code__iexact=subject_name).first()
    ...
    for index, row in df.iterrows():
        try:
            with transaction.atomic():
                ...
                course = cache['courses'][str(row.get('Course', 'MBA')).strip()]
                ...
                if not pd.isna(exam_name):
                    ...
                    res_obj, created = MBASemesterResult.objects.update_or_create(
                        student=student,
                        semester=semester,
                    ...
                    for sub_col in subject_cols:
                        ...
                        MBAStudentAssessment.objects.update_or_create(
                            student=student,
                            subject=subject_obj,
                            semester=semester,
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Import MBA Students from Excel')
