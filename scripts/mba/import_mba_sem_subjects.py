"""
Import MBA Subjects Script

This script imports MBA Semester subjects from an Excel file into the MBASubject model.
...
Then:
>>> from scripts.mba.import_mba_sem_subjects import run_import
>>> run_import('old_data/mba/MBA_SUBJECTS.xlsx')

OR run directly:
poetry run python scripts/mba/import_mba_sem_subjects.py --file "old_data/MBA_SUBJECTS.xlsx"
...
from mba_sem.models import MBASubject

def run_import(file_path):
    """
    Main entry point for importing MBA subjects.
    """
    ...
    for index, row in df.iterrows():
        try:
            ...
            subject, created = MBASubject.objects.update_or_create(
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Import MBA Subjects from Excel')
